...
***Note that these will have to be done own your own. I cannot help you with this. Refer to online search engines for this part.***
# Putting it Together
...
### Android Settings
Turn off the lock screen to use no lock under settings->lock screen and select none
### RetroArch
...
#### Hotkeys
You can set these up but on these Android handhelds the back button works well to bring up the menu. 
### Nethersx2
#### Configuration
You'll need to get aethersx2 from GitHub and then use an app called __ to decompile and add in the patches for the APK. Once finished, install the app and then start up. The prompt will need to select your BIOS otherwise you won't be able to boot. I suggest to put the save files in your SD card as well. We created that nethersx2 folder earlier so you can put it there by selecting the directory within the application. 
...
So you've gotten your games set up now and so its time to link the frontend to launch these games. Emulation Station by default will attempt to search for the various emulators available. If you need to add a custom setup, I suggest referring to the documentation from the ES-DE page. Note that you need to look for the Android specific version, not the linux version.
